...
class ItemServiceImplTest {
    ...
    void createItem_withoutItemRequest_saveItemAndReturnDto() {
        // arrange
        ItemRequestDto requestDto = Generator.makeItemRequestDtoWithoutRequestId();
    ...
    void patchItem_null_skip() {
        ...
        ItemRequestDto patch = ItemRequestDto.builder().build();
        final long itemId = item.getId();
        final long ownerId = item.getOwner().getId();
        final long requestId = itemRequest.getId();

        // act and assert
        assertThat(itemService.updateItem(itemId, patch, ownerId))
    ...
    public void createComment_validate_saveAndReturnDto() {
        ...
        when(commentRepository.save(any(Comment.class))).thenAnswer(invocationOnMock -> {
            ...
            arg.setId(1L);
            arg.setCreated(LocalDateTime.now());
            return arg;
        });

        // Act
        CommentRequestDto commentRequestDto = new CommentRequestDto(comment.getText());
        CommentAnswerDto actual = itemService.createComment(author.getId(), commentRequestDto, item.getId());
    ...
    void checkLastBookedByBookerIdAndItemId_notFound_throwException() {
        // Arrange
        CommentRequestDto commentDto = new CommentRequestDto("new comment");
        long authorId = 1L;
        long itemId = 1L;
    ...
    void findByIdAndOwnerId_notFound_throwException() {
        // Arrange
        ItemRequestDto patchDto = ItemRequestDto.builder().build();
        long ownerId = 1L;
        long itemId = 1L;

        // Act and assert
        assertThrows(NotFoundException.class, () -> itemService.updateItem(ownerId, patchDto, itemId));
    }
